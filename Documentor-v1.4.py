...
# List of popular programming/scripting/coding file extensions
code_file_extensions = [
    '.php', '.py', '.js', '.java', '.c', '.cpp', '.cs', '.ts', '.swift', '.go',
    '.rb', '.lua', '.groovy', '.scala', '.kt', '.rs', '.dart', '.sh', '.bat',
    '.pl', '.h', '.m', '.sql', '.css', '.sass', '.scss', '.less',
        ...
        for file in files:
            ...
            with open(source_file_path, "r", encoding='utf-8', errors='replace') as source_file, open(target_file_path, "w", encoding='utf-8') as target_file:
                if clone_mode == 2 and is_code_file(source_file_path):
                    target_file.write("Please analyze the following code to identify its purpose and functionality. Include inline comments to explain important functions, methods, and processes. Highlight how it integrates with other parts of the application or any external services it interacts with.\n\n```\n")
...
def main():
    ...
    if create_directory_tree == 'yes':
        generate_directory_trees(target_dir)  # Make sure you include the generate_directory_trees function from the previous example
